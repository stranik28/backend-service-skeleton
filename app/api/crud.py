...
async def get_user_balance(id: int, date: str = None) -> User:
    ...
    query = db.select([db.func.coalesce(db.func.sum(
        db.case([(Transaction.type == 'DEPOSIT', Transaction.amount),
                 (Transaction.type == 'WITHDRAW', -Transaction.amount)], else_=0)), 0).label('balance')]).where(
